...
jobid=${JOBIDS[$AWS_BATCH_JOB_ARRAY_INDEX]}
echo "Job id: ${jobid}"

s3fastaprefix="s3://serratus-rayan/beetles/logan_oct7_run/prodigal-concat/"

inputfilename="human-partial-retry.txt"
s3inputfile="s3://logan-cluster/first/human/partial/${inputfilename}"
s3resultprefix="s3://logan-cluster/first/human/partial/"
...
for split_file in split-chunks/split-*.fa; do
    ...
    # TODO: setup split size to prevent OOM
